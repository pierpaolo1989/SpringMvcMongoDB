...
public class UserService {
	...
	public List<User> getAll() {
		...
		while(cursor.hasNext()) {			
			...
			user.setNome(dbObject.get("nome").toString());
			user.setCognome(dbObject.get("cognome").toString());
	...
	public Boolean add(User user) {
		...
		log.debug("Adding a new book to the mongo database; Entered user_name is= " + user.getUsername());
		try {			
			DBCollection coll = MongoFactory.getCollection(db_name, db_collection);
			...
			doc.put("nome", user.getNome());
			doc.put("cognome", user.getCognome());	
	...
	public Boolean edit(User user) {
		...
		try {
			...
			edited.put("nome", user.getNome());
			edited.put("cognome", user.getCognome());
	...
	public Book findUserId(String id) {
		Book u = new Book();
		DBCollection coll = MongoFactory.getCollection(db_name, db_collection);
		...
		u.setTitle(dbo.get("title").toString());
		u.setAuthor(dbo.get("author").toString());
		u.setEditor(dbo.get("editor").toString());
		u.setIsbn(dbo.get("isbn").toString());
